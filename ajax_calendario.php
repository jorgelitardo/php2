...
switch ($_GET["accion"])
{
	...
	case "borrar_evento":
	{
		...
		if ($resultado=mysql_query($query)) 
		{
			...
			$query2= mysql_query("update postulantes set fecha_dece_estudiante = 'PENDIENTE' where reg='$codigo'",$cn);
				...
				if ($tr<$totalfilas)
				{
					if ($i>=$primeromes && $i<=$tope) 
					{
						...
						//if ($eventos_del_dia!="")
						{
								...
								//$psicologo= $_SESSION['psicologo'];
								require_once("conexion2.php");
								$clavebuscada1=  mysql_query("SELECT * FROM postulantes WHERE '$cod_est' = reg and (estado = 'POR PAGAR' or estado = 'FECHA_DECE' or estado = 'DECE')");
								...
								$clavebuscada=  mysql_query("SELECT * FROM tbl_turnos WHERE codigo NOT IN (SELECT turno FROM tbl_calendario where fecha = '$fecha_completa' and psicologo = '$psicologo' and estado = 'A') and estado = 'A'");
